# xtensa_loadstore_handler.S 代码解说

源文件：`portable/ThirdParty/GCC/Xtensa_ESP32/xtensa_loadstore_handler.S`

> 本文件由 `tools/generate_code_markdown.py` 自动生成。内容是面向阅读的代码解说，不会替代源码注释或官方文档。

## 片段 1: 文件头和许可证

```asm
/*
 * SPDX-FileCopyrightText: 2015-2022 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Apache-2.0
 */
```

**解说：** 这一段是文件头，说明项目归属、许可证和免责条款；它告诉使用者这个文件按 MIT 许可证发布。

## 片段 2: 说明性注释

```asm
/*
 *  LoadStoreErrorCause:     Occurs when trying to access 32 bit addressable memory region as 8 bit or 16 bit
 *  LoadStoreAlignmentCause: Occurs when trying to access in an unaligned manner
 *
 *          xxxx xxxx = imm8 field
 *               yyyy = imm4 field
 *               ssss = s field
 *               tttt = t field
 *
 *                            16                0
 *                            -------------------
 *          L32I.N            yyyy ssss tttt 1000
 *          S32I.N            yyyy ssss tttt 1001
 *
 *                  23                          0
 *                  -----------------------------
 *          L8UI    xxxx xxxx 0000 ssss tttt 0010               <- LoadStoreError
 *          L16UI   xxxx xxxx 0001 ssss tttt 0010               <- LoadStoreError, LoadStoreAlignment
 *          L16SI   xxxx xxxx 1001 ssss tttt 0010               <- LoadStoreError, LoadStoreAlignment
 *          L32I    xxxx xxxx 0010 ssss tttt 0010               <- LoadStoreAlignment
 *
 *          S8I     xxxx xxxx 0100 ssss tttt 0010               <- LoadStoreError
 *          S16I    xxxx xxxx 0101 ssss tttt 0010               <- LoadStoreError, LoadStoreAlignment
 *          S32I    xxxx xxxx 0110 ssss tttt 0010               <- LoadStoreAlignment
 *
 *                   ******* UNSUPPORTED *******
 *
 *          L32E    0000 1001 rrrr ssss tttt 0000
 *          S32E    0100 1001 rrrr ssss tttt 0000
 *                  -----------------------------
 */
```

**解说：** 这一段是源码作者留下的说明，概括了后续代码的意图或使用条件。原意可理解为：LoadStoreErrorCause: Occurs when trying to access 32 bit addressable memory region as 8 bit or 16 bit LoadStoreAlignmentCause: Occurs when trying to access in an unaligned manner xxxx xxxx = imm8 field yyyy = imm4 field ssss = s field tttt 。

## 片段 3: 预处理配置

```asm
#include "xtensa_rtos.h"
#include "sdkconfig.h"
#include "soc/soc.h"
```

**解说：** 这一段引入当前文件依赖的头文件，让后续代码可以使用 FreeRTOS、标准库或移植层提供的类型、宏和函数。

## 片段 4: 汇编标签 LOADSTORE_HANDLER_STACK_SZ

```asm
#define LOADSTORE_HANDLER_STACK_SZ      8
    .section .bss, "aw"
    .balign 16
LoadStoreHandlerStack:
    .rept LOADSTORE_HANDLER_STACK_SZ
    .word 0
    .endr
```

**解说：** 这一段是汇编标签 `LOADSTORE_HANDLER_STACK_SZ` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 5: 说明性注释

```asm

/* LoadStoreErrorCause handler:
 *
 * Completes 8-bit or 16-bit load/store instructions from 32-bit aligned memory region
 * Called from UserExceptionVector if EXCCAUSE is LoadStoreErrorCause
 */
```

**解说：** 这一段是源码作者留下的说明，概括了后续代码的意图或使用条件。原意可理解为：LoadStoreErrorCause handler: Completes 8-bit or 16-bit load/store instructions from 32-bit aligned memory region Called from UserExceptionVector if EXCCAUSE is LoadStoreErrorCause。

## 片段 6: 代码片段 6

```asm
    .global   LoadStoreErrorHandler
    .section .iram1, "ax"
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 7: 代码片段 7

```asm
    .literal_position
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 8: 汇编标签 LoadStoreErrorHandler

```asm
    .balign 4
LoadStoreErrorHandler:
    .type   LoadStoreErrorHandler, @function
```

**解说：** 这一段是汇编标签 `LoadStoreErrorHandler` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 9: 代码片段 9

```asm
    wsr     a0, depc                            // Save return address in depc
    mov     a0, sp
    movi    sp, LoadStoreHandlerStack
    s32i    a0, sp, 0x04                        // Since a0 contains value of a1
    s32i    a2, sp, 0x08
    s32i    a3, sp, 0x0c
    s32i    a4, sp, 0x10
```

**解说：** 这一段计算并返回结果；调用者会根据返回值继续决定后续流程。

## 片段 10: 代码片段 10

```asm
    rsr     a0, sar                             // Save SAR in a0 to restore later
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 11: 代码片段 11

```asm
    /* Check whether the address lies in the valid range */
    rsr     a3, excvaddr
    movi    a4, _iram_text_end                  // End of code section of IRAM
    bge     a3, a4, 1f
    movi    a4, SOC_CACHE_APP_LOW               // Check if in APP cache region
    blt     a3, a4, .LS_wrong_opcode
    movi    a4, SOC_CACHE_APP_HIGH
    bge     a3, a4, .LS_wrong_opcode
    j       2f
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 12: 代码片段 12

```asm
1:
    movi    a4, SOC_IRAM_HIGH                   // End of IRAM address range
    bge     a3, a4, .LS_wrong_opcode
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 13: 代码片段 13

```asm
2:
    /* Examine the opcode which generated the exception */
    /* Note: Instructions are in this order to avoid pipeline stalls. */
    rsr     a2, epc1
    movi    a4, ~3
    ssa8l   a2                                  // sar is now correct shift for aligned read
    and     a2, a2, a4                          // a2 now 4-byte aligned address of instruction
    l32i    a4, a2, 0
    l32i    a2, a2, 4
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 14: 代码片段 14

```asm
    src     a2, a2, a4                          // a2 now instruction that failed
    bbci    a2, 1, .LS_wrong_opcode
    bbsi    a2, 14, .LSE_store_op               // Store instruction
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 15: 代码片段 15

```asm
    /* l8/l16ui/l16si */
    movi    a4, ~3
    and     a4, a3, a4                          // a4 now word aligned read address
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 16: 代码片段 16

```asm
    ssa8l   a3                                  // sar is now shift to extract a3's byte
    l32i    a4, a4, 0                           // perform the actual read
    srl     a4, a4                              // shift right correct distance
    extui   a3, a2, 12, 4
    bnez    a3, 1f                              // l16ui/l16si
    extui   a4, a4, 0, 8                        // mask off bits needed for an l8
    j       2f
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 17: 代码片段 17

```asm
1:
    extui   a4, a4, 0, 16
    bbci    a2, 15, 2f                          // l16ui
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 18: 代码片段 18

```asm
    /* Sign adjustment */
    slli    a4, a4, 16
    srai    a4, a4, 16                          // a4 contains the value
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 19: 代码片段 19

```asm
2:
    /* a4 contains the value */
    rsr     a3, epc1
    addi    a3, a3, 3
    wsr     a3, epc1
    wsr     a0, sar
    rsr     a0, excsave1
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 20: 代码片段 20

```asm
    extui   a2, a2, 3, 5
    blti    a2, 10, .LSE_stack_reg
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 21: 代码片段 21

```asm
    movi    a3, .LS_jumptable_base
    addx8   a2, a2, a3                          // a2 is now the address to jump to
    l32i    a3, sp, 0x0c
    jx      a2
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 22: 汇编标签 .LSE_stack_reg

```asm
.LSE_stack_reg:
    addx2   a2, a2, sp
    s32i    a4, a2, 0
```

**解说：** 这一段是汇编标签 `.LSE_stack_reg` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 23: 代码片段 23

```asm
    /* Restore all values */
    l32i    a4, sp, 0x10
    l32i    a3, sp, 0x0c
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 24: 汇编标签 .LSE_store_op

```asm
.LSE_store_op:
    s32i    a5, a1, 0x14
    s32i    a6, a1, 0x18
```

**解说：** 这一段是汇编标签 `.LSE_store_op` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 25: 代码片段 25

```asm
    /* a2 -> instruction that caused the error */
    /* a3 -> unaligned address */
    extui   a4, a2, 4, 4
    blti    a4, 7, 1f
    movi    a5, .LSE_store_reg
    addx8   a5, a4, a5
    jx      a5
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 26: 代码片段 26

```asm
1:
    addx4   a4, a4, sp
    l32i    a4, a4, 0
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 27: 汇编标签 .LSE_store_data

```asm
.LSE_store_data:
    /* a4 contains the value */
    rsr     a6, epc1
    addi    a6, a6, 3
    wsr     a6, epc1
```

**解说：** 这一段是汇编标签 `.LSE_store_data` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 28: 代码片段 28

```asm
    ssa8b   a3
    movi    a5, -1
    bbsi    a2, 12, 1f                          // s16
    extui   a4, a4, 0, 8
    movi    a6, 0xff
    j       2f
1:
    extui   a4, a4, 0, 16
    movi    a6, 0xffff
2:
    sll     a4, a4                              // shift the value to proper offset
    sll     a6, a6
    xor     a5, a5, a6                          // a5 contains the mask
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 29: 代码片段 29

```asm
    movi    a6, ~3
    and     a3, a3, a6                          // a3 has the aligned address
    l32i    a6, a3, 0                           // a6 contains the data at the aligned address
    and     a6, a6, a5
    or      a4, a6, a4
    s32i    a4, a3, 0
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 30: 代码片段 30

```asm
    /* Restore registers */
    wsr     a0, sar
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 31: 代码片段 31

```asm
    l32i    a6, sp, 0x18
    l32i    a5, sp, 0x14
    l32i    a4, sp, 0x10
    l32i    a3, sp, 0x0c
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rsr     a0, excsave1
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 32: 代码片段 32

```asm
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 33: 汇编标签 .LSE_store_reg

```asm
.LSE_store_reg:
    .org .LSE_store_reg + (7 * 8)
    mov     a4, a7
    j .LSE_store_data
```

**解说：** 这一段是汇编标签 `.LSE_store_reg` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 34: 代码片段 34

```asm
    .org .LSE_store_reg + (8 * 8)
    mov     a4, a8
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 35: 代码片段 35

```asm
    .org .LSE_store_reg + (9 * 8)
    mov     a4, a9
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 36: 代码片段 36

```asm
    .org .LSE_store_reg + (10 * 8)
    mov     a4, a10
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 37: 代码片段 37

```asm
    .org .LSE_store_reg + (11 * 8)
    mov     a4, a11
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 38: 代码片段 38

```asm
    .org .LSE_store_reg + (12 * 8)
    mov     a4, a12
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 39: 代码片段 39

```asm
    .org .LSE_store_reg + (13 * 8)
    mov     a4, a13
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 40: 代码片段 40

```asm
    .org .LSE_store_reg + (14 * 8)
    mov     a4, a14
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 41: 代码片段 41

```asm
    .org .LSE_store_reg + (15 * 8)
    mov     a4, a15
    j .LSE_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 42: 说明性注释

```asm

/* LoadStoreAlignmentCause handler:
 *
 * Completes unaligned 16-bit and 32-bit load/store instructions from 32-bit aligned memory region
 * Called from UserExceptionVector if EXCCAUSE is LoadStoreAlignmentCause
 */
```

**解说：** 这一段是源码作者留下的说明，概括了后续代码的意图或使用条件。原意可理解为：LoadStoreAlignmentCause handler: Completes unaligned 16-bit and 32-bit load/store instructions from 32-bit aligned memory region Called from UserExceptionVector if EXCCAUSE is LoadStoreAlignmentCause。

## 片段 43: 代码片段 43

```asm
    .global   AlignmentErrorHandler
    .section .iram1, "ax"
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 44: 代码片段 44

```asm
    .literal_position
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 45: 汇编标签 AlignmentErrorHandler

```asm
    .balign 4
AlignmentErrorHandler:
    .type   AlignmentErrorHandler, @function
```

**解说：** 这一段是汇编标签 `AlignmentErrorHandler` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 46: 代码片段 46

```asm
    wsr     a0, depc                            // Save return address in depc
    mov     a0, sp
    movi    sp, LoadStoreHandlerStack
    s32i    a0, sp, 0x04                        // Since a0 contains value of a1
    s32i    a2, sp, 0x08
    s32i    a3, sp, 0x0c
    s32i    a4, sp, 0x10
```

**解说：** 这一段计算并返回结果；调用者会根据返回值继续决定后续流程。

## 片段 47: 代码片段 47

```asm
    rsr     a0, sar                             // Save SAR in a0 to restore later
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 48: 代码片段 48

```asm
    /* Check whether the address lies in the valid range */
    rsr     a3, excvaddr
    movi    a4, _iram_text_end                  // End of code section of IRAM
    bge     a3, a4, 1f
    movi    a4, SOC_CACHE_APP_LOW               // Check if in APP cache region
    blt     a3, a4, .LS_wrong_opcode
    movi    a4, SOC_CACHE_APP_HIGH
    bge     a3, a4, .LS_wrong_opcode
    j       2f
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 49: 代码片段 49

```asm
1:
    movi    a4, SOC_IRAM_HIGH                   // End of IRAM address range
    bge     a3, a4, .LS_wrong_opcode
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 50: 代码片段 50

```asm
2:
    /* Examine the opcode which generated the exception */
    /* Note: Instructions are in this order to avoid pipeline stalls. */
    rsr     a2, epc1
    movi    a4, ~3
    ssa8l   a2                                  // sar is now correct shift for aligned read
    and     a2, a2, a4                          // a2 now 4-byte aligned address of instruction
    l32i    a4, a2, 0
    l32i    a2, a2, 4
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 51: 代码片段 51

```asm
    /* a2 has the instruction that caused the error */
    src     a2, a2, a4
    extui   a4, a2, 0, 4
    addi    a4, a4, -9
    beqz    a4, .LSA_store_op
    bbsi    a2, 14, .LSA_store_op
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 52: 代码片段 52

```asm
    ssa8l   a3                                  // a3 contains the unaligned address
    movi    a4, ~3
    and     a4, a3, a4                          // a4 has the aligned address
    l32i    a3, a4, 0
    l32i    a4, a4, 4
    src     a4, a4, a3
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 53: 代码片段 53

```asm
    rsr     a3, epc1
    addi    a3, a3, 2
    bbsi    a2, 3, 1f                           // l32i.n
    bbci    a2, 1, .LS_wrong_opcode
    addi    a3, a3, 1
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 54: 代码片段 54

```asm
    bbsi    a2, 13, 1f                          // l32
    extui   a4, a4, 0, 16
    bbci    a2, 15, 1f                          // l16ui
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 55: 代码片段 55

```asm
    /* Sign adjustment */
    slli    a4, a4, 16
    srai    a4, a4, 16                          // a4 contains the value
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 56: 代码片段 56

```asm
1:
    wsr     a3, epc1
    wsr     a0, sar
    rsr     a0, excsave1
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 57: 代码片段 57

```asm
    extui   a2, a2, 4, 4
    blti    a2, 5, .LSA_stack_reg               // a3 contains the target register
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 58: 代码片段 58

```asm
    movi    a3, .LS_jumptable_base
    slli    a2, a2, 4
    add     a2, a2, a3                          // a2 is now the address to jump to
    l32i    a3, sp, 0x0c
    jx      a2
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 59: 汇编标签 .LSA_stack_reg

```asm
.LSA_stack_reg:
    addx4   a2, a2, sp
    s32i    a4, a2, 0
```

**解说：** 这一段是汇编标签 `.LSA_stack_reg` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 60: 代码片段 60

```asm
    /* Restore all values */
    l32i    a4, sp, 0x10
    l32i    a3, sp, 0x0c
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 61: 汇编标签 .LSA_store_op

```asm
/* Store instruction */
.LSA_store_op:
    s32i    a5, sp, 0x14
    s32i    a6, sp, 0x18
    s32i    a7, sp, 0x1c
```

**解说：** 这一段是汇编标签 `.LSA_store_op` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 62: 代码片段 62

```asm
    /* a2 -> instruction that caused the error */
    /* a3 -> unaligned address */
    extui   a4, a2, 4, 4
    blti    a4, 8, 1f
    movi    a5, .LSA_store_reg
    addx8   a5, a4, a5
    jx      a5
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 63: 代码片段 63

```asm
1:
    addx4   a4, a4, sp
    l32i    a4, a4, 0                           // a4 contains the value
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 64: 汇编标签 .LSA_store_data

```asm
.LSA_store_data:
    movi    a6, 0
```

**解说：** 这一段是汇编标签 `.LSA_store_data` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 65: 代码片段 65

```asm
    rsr     a7, epc1
    addi    a7, a7 ,2
    bbsi    a2, 3, 1f                           // s32i.n
    bbci    a2, 1, .LS_wrong_opcode
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 66: 代码片段 66

```asm
    addi    a7, a7, 1
    bbsi    a2, 13, 1f                          // s32i
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 67: 代码片段 67

```asm
    movi    a5, -1
    extui   a4, a4, 0, 16
    slli    a6, a5, 16                          // 0xffff0000
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 68: 代码片段 68

```asm
1:
    wsr     a7, epc1
    movi    a5, ~3
    and     a5, a3, a5                          // a5 has the aligned address
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 69: 代码片段 69

```asm
    ssa8b   a3
    movi    a3, -1
    src     a7, a6, a3
    src     a3, a3, a6
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 70: 代码片段 70

```asm
    /* Store data on lower address */
    l32i    a6, a5, 0
    and     a6, a6, a7
    sll     a7, a4
    or      a6, a6, a7
    s32i    a6, a5, 0
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 71: 代码片段 71

```asm
    /* Store data on higher address */
    l32i    a7, a5, 4
    srl     a6, a4
    and     a3, a7, a3
    or      a3, a3, a6
    s32i    a3, a5, 4
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 72: 代码片段 72

```asm
    /* Restore registers */
    wsr     a0, sar
    rsr     a0, excsave1
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 73: 代码片段 73

```asm
    l32i    a7, sp, 0x1c
    l32i    a6, sp, 0x18
    l32i    a5, sp, 0x14
    l32i    a4, sp, 0x10
    l32i    a3, sp, 0x0c
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 74: 汇编标签 .LSA_store_reg

```asm
.LSA_store_reg:
    .org .LSA_store_reg + (8 * 8)
    mov     a4, a8
    j       .LSA_store_data
```

**解说：** 这一段是汇编标签 `.LSA_store_reg` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 75: 代码片段 75

```asm
    .org .LSA_store_reg + (9 * 8)
    mov     a4, a9
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 76: 代码片段 76

```asm
    .org .LSA_store_reg + (10 * 8)
    mov     a4, a10
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 77: 代码片段 77

```asm
    .org .LSA_store_reg + (11 * 8)
    mov     a4, a11
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 78: 代码片段 78

```asm
    .org .LSA_store_reg + (12 * 8)
    mov     a4, a12
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 79: 代码片段 79

```asm
    .org .LSA_store_reg + (13 * 8)
    mov     a4, a13
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 80: 代码片段 80

```asm
    .org .LSA_store_reg + (14 * 8)
    mov     a4, a14
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 81: 代码片段 81

```asm
    .org .LSA_store_reg + (15 * 8)
    mov     a4, a15
    j       .LSA_store_data
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 82: 汇编标签 .LS_jumptable

```asm
/*
 * Common routines for both the exception handlers
 */
    .balign 4
.LS_jumptable:
    /*  The first 5 entries (80 bytes) of this table are unused (registers
        a0..a4 are handled separately above).  Rather than have a whole bunch
        of wasted space, just pretend that the table starts 80 bytes
        earlier in memory. */
    .set    .LS_jumptable_base, .LS_jumptable - (16 * 5)
```

**解说：** 这一段是汇编标签 `.LS_jumptable` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。

## 片段 83: 代码片段 83

```asm
    .org    .LS_jumptable_base + (16 * 5)
    mov     a5, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 84: 代码片段 84

```asm
    .org    .LS_jumptable_base + (16 * 6)
    mov     a6, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 85: 代码片段 85

```asm
    .org    .LS_jumptable_base + (16 * 7)
    mov     a7, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 86: 代码片段 86

```asm
    .org    .LS_jumptable_base + (16 * 8)
    mov     a8, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 87: 代码片段 87

```asm
    .org    .LS_jumptable_base + (16 * 9)
    mov     a9, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 88: 代码片段 88

```asm
    .org    .LS_jumptable_base + (16 * 10)
    mov     a10, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 89: 代码片段 89

```asm
    .org    .LS_jumptable_base + (16 * 11)
    mov     a11, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 90: 代码片段 90

```asm
    .org    .LS_jumptable_base + (16 * 12)
    mov     a12, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 91: 代码片段 91

```asm
    .org    .LS_jumptable_base + (16 * 13)
    mov     a13, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 92: 代码片段 92

```asm
    .org    .LS_jumptable_base + (16 * 14)
    mov     a14, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 93: 代码片段 93

```asm
    .org    .LS_jumptable_base + (16 * 15)
    mov     a15, a4
    l32i    a4, sp, 0x10
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rfe
```

**解说：** 这一段是 `xtensa_loadstore_handler.S` 中的普通实现代码，负责准备数据、更新状态或串联前后逻辑，使所在模块能够完成自己的职责。

## 片段 94: 汇编标签 .LS_wrong_opcode

```asm
.LS_wrong_opcode:
    /* Reaches here if the address is in invalid range or the opcode isn't supported.
     * Restore registers and jump back to _xt_user_exc
     */
    wsr     a0, sar
    l32i    a4, sp, 0x10
    l32i    a3, sp, 0x0c
    l32i    a2, sp, 0x08
    l32i    a1, sp, 0x04
    rsr     a0, depc
    ret                                         // Equivalent to jx a0
```

**解说：** 这一段是汇编标签 `.LS_wrong_opcode` 附近的代码，通常对应异常入口、上下文切换、启动流程或特定处理器指令序列。
